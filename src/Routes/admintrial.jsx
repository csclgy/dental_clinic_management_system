        ...
        <div className="row">
          ...
          >
            ...
            >
              ...
              <i className={`fa fa-chevron-${isLedgerOpen ? "up" : "down"}`} aria-hidden="true"></i>
            </button>

            {isLedgerOpen && (
              <div className="ml-8 text-sm">
                <Link to="/admincoa">
                  <p className="py-1 hover:underline" style={{ color: "#00458B" }}>Chart of Accounts</p>
                </Link>
                <Link to="/adminjournal">
                  <p className="py-1 hover:underline" style={{ color: "#00458B" }}>Journal Entries</p>
                </Link>
                <Link to="/adminsubsidiaryreceivable">
                  <p className="py-1 hover:underline" style={{ color: "#00458B" }}>Subsidiary</p>
                </Link>
                <Link to="/admingeneral">
                  <p className="py-1 hover:underline" style={{ color: "#00458B" }}>General Ledger</p>
                </Link>
                <Link to="/admintrial">
                  <p className="py-1 hover:underline" style={{ color: "#00c3b8" }}>Trial Balance</p>
                </Link>
              </div>
            )}
          ...
          <div className="col-sm-7">
            <div className="row">
              ...
              <div className="col-sm-12 p-10 rounded-lg shadow-lg mt-4" style={{ border: "solid", borderColor: "#01D5C4" }}>
                    ...
                    <tbody>
                      {filteredRecords.length > 0 ? (
                        filteredRecords.map((record, index) => (
                          <tr key={index} className="border-b border-gray-200">
                            ...
                            <td className="px-4 py-2 text-blue-700"> {Number(record.debit).toLocaleString("en-PH", { minimumFractionDigits: 2 })} </td>
                            <td className="px-4 py-2 text-blue-700"> {Number(record.credit).toLocaleString("en-PH", { minimumFractionDigits: 2 })} </td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                     ...
                     < div className="flex justify-between mt-6 text-[#00458B] font-bold">
                      ...
                      <div className="flex gap-12">
                        <span>₱ {Number(totalDebit).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</span> 
                        <span>₱ {Number(totalCredit).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</span>
                      </div>
                    </div>
                </div>
