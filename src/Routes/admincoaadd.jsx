...
const AdminCoaAdd = () => {
  ...
  // Save COA with validation
  const handleSave = async () => {
    if (!accountName.trim()) {
      setAlertMessage({ text: "Account Name is required!", type: "error" });
    ...
    try {
      ...
      setAlertMessage({
        text: response.data.message || "Account saved successfully!",
        type: "success",
      });
      setAccountName("");
  ...
  // ✅ AlertBox (Success = green, Error = red)
  ...
  return (
    <div>
      ...
      {alertMessage && (
        <AlertBox message={alertMessage.text} type={alertMessage.type} />
      )}
    </div>
  );
};
